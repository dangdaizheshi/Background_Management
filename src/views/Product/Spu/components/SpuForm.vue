  ...
  <el-form label-width="100px">
    ...
    <el-form-item label="SPU销售属性">
        <el-select style="width: 240px; margin-right:10px;">
            <el-option :label="item.name" v-for="(item, index) in allSaleAttr" :key="index"></el-option>
        </el-select>
        <el-button type="primary" icon="Plus">添加属性值</el-button>
        <el-table border style="margin: 15px 0;" :data="saleAttr">
          <el-table-column label="序号" width="80px" align="center" type="index"></el-table-column>
          <el-table-column label="属性名" width="120px" prop="saleAttrName"></el-table-column>
          <el-table-column label="属性值">
            <template #default="scope">
                <el-tag v-for="item in scope.row.spuSaleAttrValueList" :key="scope.row.id" closable style="margin: 5px;">
          ...
          <el-table-column label="操作" width="120px">
            <template #default="scope">
              <el-button type="danger" icon="Delete" size="small" title="删除SPU" @click="saleAttr.splice(scope.$index, 1)"></el-button>
            </template>
          </el-table-column>
        </el-table>
    </el-form-item>
...
const initSpuData = async(spu: SpuData) => {
  ...
  allImgList.value = res2.data.map(item => {
    return {
        name: item.imgName,
        url: item.imgUrl
    } as SkuImg
